...
description: Learn how to use OpenAI functions with the Vercel AI SDK.
---
...
```ts highlight="2,3,4,15,17,21,22"
const stream = OpenAIStream(response, {
...
<Note>
  When using the `streamData` API, the assistant message containing the
  `function_call` has no `content`. Previously, it contained the stringified
